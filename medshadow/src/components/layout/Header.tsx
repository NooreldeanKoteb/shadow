  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[99999] flex items-center justify-between px-6 md:px-10 py-2 transition-all duration-300 ${scrolled ? 'bg-white/70 backdrop-blur border-b border-gray-200 shadow-md' : 'bg-transparent'}`}>
      ...
      <nav className="hidden md:flex items-center space-x-6">
        ...
        <Link href="/auth/signin" className="bg-[#FCA311] text-white rounded px-6 py-2 font-medium ml-2 flex items-center h-10 hover:bg-[#FCA311]/90 transition-colors">Sign In</Link>
      </nav>
      {/* Mobile menu button */}
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 mt-0 py-4 bg-white shadow-lg border-b border-gray-200 z-[99999]">
          <nav className="flex flex-col space-y-1 px-4">
            ...
            <Link href="/auth/signin" className="block bg-[#FCA311] text-white rounded px-6 py-3 font-medium mt-2 text-center hover:bg-[#FCA311]/90 transition-colors">Sign In</Link>
          </nav>
        </div>
      )}
    </header>
